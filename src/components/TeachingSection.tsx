...
interface TeachingSectionProps {
    data: {
        ...
        professional_memberships: Array<{ organization: string; membership_id?: string }>;
        personal_info?: {
            experience_summary?: {
                teaching: number;
            };
...
export default function TeachingSection({ data }: TeachingSectionProps) {
                ...
                <div ref={contentRef} className="stagger-children" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
                    ...
                    <div style={{ backgroundColor: '#FAFAFA', border: '1px solid #E4E4E7', padding: '32px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            {(subjects_handled || []).map((subject, i) => (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                {(professional_memberships || []).map((m) => (
                                    <div key={m.organization} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', backgroundColor: '#F4F4F5', border: '1px solid #E4E4E7' }}>
                                        ...
                                        {m.membership_id && <span style={{ fontSize: '11px', color: '#71717A', fontFamily: 'Space Grotesk, monospace' }}>#{m.membership_id}</span>}
                                    </div>
                                ))}
                            </div>
                        </div>
