...
function MetricCard({ value, label, sub }: { value: string | number; label: string; sub?: string }) {
    return (
        <div style={{ padding: '24px', border: '1px solid #E4E4E7', backgroundColor: '#FAFAFA', transition: 'box-shadow 0.2s ease, transform 0.2s ease', cursor: 'default' }}
            ...
            <div style={{ fontFamily: 'Archivo, sans-serif', fontSize: '40px', fontWeight: 800, color: '#09090B', lineHeight: 1 }}>{value}</div>
...
interface ResearchSectionProps {
    data: {
        publications: { total_papers: number; patents: number; books_authored: number; copyrights: number };
        major_research_projects: Array<{ title: string; role: string; funding_agency: string; amount_lakhs: number; year: number }>;
        personal_info: {
            google_scholar: { documents: number; citations: number; h_index: number; i10_index: number; link: string };
            scopus: { documents: number; citations: number; h_index: number; link: string };
            orcid: string;
        };
            ...
            <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
                ...
                <div style={{ textAlign: 'left', marginBottom: '60px' }}>
                    ...
                    <h2 style={{
                        ...
                        }}>{intro.title_line_2 || 'Measurable Impact'}</span>
                    </h2>
                </div>
                ...
                <div ref={metricsRef} className="stagger-children" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px', marginBottom: '80px' }}>
                    ...
                    <div style={{ backgroundColor: '#09090B', color: '#FAFAFA', border: '1px solid #27272A', padding: '36px' }}>
                        ...
                        {[
                            { label: 'Total Documents', value: personal_info.google_scholar.documents },
                            { label: 'Total Citations', value: personal_info.google_scholar.citations },
                            { label: 'H-Index', value: personal_info.google_scholar.h_index },
                            { label: 'i10-Index', value: personal_info.google_scholar.i10_index },
                        ].map(({ label, value }) => (
                    ...
                    <div style={{ backgroundColor: '#09090B', color: '#FAFAFA', border: '1px solid #27272A', padding: '36px' }}>
                        ...
                        {[
                            { label: 'Total Documents', value: personal_info.scopus.documents },
                            { label: 'Total Citations', value: personal_info.scopus.citations },
                            { label: 'H-Index', value: personal_info.scopus.h_index },
                            { label: 'ORCID', value: personal_info.orcid },
                        ].map(({ label, value }) => (
                        ...
                        <a href={personal_info.scopus.link} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', marginTop: '20px', fontSize: '12px', color: '#D97706', textDecoration: 'none', fontWeight: 500 }}>
                            View Profile <ExternalLink size={12} />
                        </a>
                    </div>
                </div>
                ...
                <div ref={projectsRef} className="reveal">
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', backgroundColor: '#E4E4E7', border: '1px solid #E4E4E7' }}>
                        {major_research_projects.map((project) => (
                            <div key={project.title} style={{ backgroundColor: '#FAFAFA', padding: '28px 32px', display: 'grid', gridTemplateColumns: '1fr auto', gap: '16px', alignItems: 'center', transition: 'background-color 0.2s ease', cursor: 'default' }}
                                onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#F4F4F5'}
                                onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#FAFAFA'}>
                                <div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', flexWrap: 'wrap' }}>
                                        <span style={{ fontSize: '10px', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#DC2626', border: '1px solid #DC262640', backgroundColor: '#DC262610', padding: '2px 8px' }}>{project.funding_agency}</span>
                                        <span style={{ fontSize: '11px', color: '#71717A' }}>{project.year}</span>
                                        <span style={{ fontSize: '11px', color: '#52525B' }}>{project.role}</span>
                                    </div>
                                    <div style={{ fontFamily: 'Archivo, sans-serif', fontWeight: 700, fontSize: '15px', color: '#09090B', lineHeight: 1.4 }}>{project.title}</div>
                                </div>
                                <div style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', justifyContent: 'flex-end' }}>
                                        <Lightbulb size={14} color="#D97706" />
                                        <span style={{ fontFamily: 'Archivo, sans-serif', fontSize: '20px', fontWeight: 800, color: '#09090B' }}>₹{project.amount_lakhs}L</span>
                                    </div>
